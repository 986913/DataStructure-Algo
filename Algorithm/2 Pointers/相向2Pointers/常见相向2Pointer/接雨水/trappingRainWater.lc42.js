...
/******************************** Solution 2: Two pointer - LC11变形题 ********************************************** */
/*
  这个算法的核心思想是从两侧往中间遍历，每次选择较小的一侧移动，
  并在遍历过程中维护左侧最大值leftMax 和右侧最大值rightMax，以确保计算接雨水量的准确性
*/
var trap = function (height) {
  ...
  while (left < right) {
    if (height[left] < height[right]) {
      ...
      result += leftMax - height[left]; //计算当前位置上的接雨水量

      left++;
    } else {
      rightMax = Math.max(rightMax, height[right]);
      result += rightMax - height[right]; //计算当前位置上的接雨水量
