...
2. 确定递推公式:              dp[i] += dp[j - 1] * dp[i - j];       
                            j表示当前头结点, j-1为是左子树节点数量，i-j是右子树节点数量。
3. dp数组如何初始化:          dp[0]=1, dp[1]=1
4. 确定遍历顺序: 
                            首先一定是遍历节点数，从递归公式：dp[i] += dp[j-1] * dp[i-j]可以看出，
                            节点数为i的状态是依靠i之前节点数的状态。那么遍历i里面每一个数作为头结点的状态，用j来遍历。
                              for (int i = 2; i <= n; i++) {
                                for (int j = 1; j <= i; j++) {
                                    dp[i] += dp[j - 1] * dp[i - j];
                                }
...
var numTrees = function (n) {
  ...
  //外循环：i表示有i个节点，因为要从1个节点开始累计，后面的节点要用到前面的节点
  for (let i = 2; i <= n; i++) {
    //内循环：这i个节点时 以j为头节点。一旦j确定，那么由于BST的性质, 其左子树的构造方式共有dp[j-1]种, 其右子树的构造方式共有dp[i-j]种
    for (let j = 1; j <= i; j++) {
      dp[i] += dp[j - 1] * dp[i - j];
    }
  }
